...
// Commands processed via sscanf(RXBuffer, "%f %f %f %f %d %d %d %d %d %d", &volt3, &volt2, &volt4, &volt1, &adcnum, &nreq, &nfreq, &atten, &onoff, &gain);
...
//unsigned modadd = 0x03; // NMR module address set arbitrarily to 2 for now, will eventually be passed as calling argument
...
int ADCave[1000]; // average voltage
...
// use inline code here to increase speed, as done for ADC
{
	int x;
	unsigned long rcv = 0;
	SYNCdac(0);  // starts DAC
	if (d&0x800000) state = state | sdatadac; // pick msb of transmit word to DAC and write bit to databus
	else state = state & ~sdatadac;
	outportb(state);
	for (x = 24; x; x--) {
		sim.gpio.podr_datal = state & ~sclockdac; // write DAC clock to netburner data bus, inline code for best speed
		sim.gpio.podr_datah = addr_state | dstb; // write data strobe to address bus
		sim.gpio.podr_datah = addr_state & ~dstb; // clear data strobe
		d <<= 1; // shift transmit bit left
		if (d&0x800000) state = (state | sdatadac); // pick next bit of transmit word and combine without clock bit
		else state = (state & ~sdataadc);
		sim.gpio.podr_datal = state; // write netburner data bus
		sim.gpio.podr_datah = addr_state | dstb; // write data strobe
		sim.gpio.podr_datah = addr_state & ~dstb; // clear data strobe
...
double UserADC(int adcnum, int nreq, int nloop) { // input # reads and freq count
  ...
  unsigned long config = 0x0000;  // ADC configuration data for +- 5V range
  unsigned long ADCAddress = 0x0000; // ADC address
  int ADCValue; // 32 bit signed (2's complement) ADC value
  int ADCSum = 0;
...
void UserDAC(int dac1, float volt1, int dac2, float volt2) {
	...
	// unsigned long output = 0x0C0005; // Output range data +-10.8V
	...
	DACValue = int((65536.*(volt1 + 10.8)) / 21.6) ^ 0x8000; // convert voltage to digital, 2's complement, XOR of msb
	DACAddress = 0x010000 * dac1;
...
/*	if (first) { // only initialize once
	...
	if (!(dac1==dac2 && volt1==volt2)) {
		DACValue = int((65536.*(volt2 + 10.8)) / 21.6) ^ 0x8000; //2's complement, XOR of msb
		DACAddress = 0x010000 * dac2;
...
// create a special version for FM DAC4
void UserDACFM(int dac4, int nfreq, int nloop) {
	static unsigned long DACdata[1000]; // data contains the DAC address and requested voltage
	...
	unsigned char port = 0x01; // DAC port address
	unsigned long readword;  //readback data
	static int oldnfreq = 0;

	if (nfreq != oldnfreq) { // create new DAC data array with nfreq steps of -1 to +1V
		oldnfreq = nfreq;
		DACAddress = 0x010000 * dac4;
		loop = 0;
		voltage = -1.;
		vstep = 2./(float(nfreq)-1); // call routine to precompute DAC values if vstep has changed
		while (loop < nfreq+1) {
			DACValue = int((65536.*(voltage + 10.8)) / 21.6) ^ 0x8000;
			DACdata[loop] = DACAddress | DACValue;
			voltage = voltage + vstep;
			//printf("DAC %d, freq %d, voltage %-6.3f, DACval %x \n", dac4, loop, voltage, DACdata[loop]);
			loop++;
		}
	}
	setaddr(port); // select port # to access via address register and switch to output mode
	spidac_start();              // initialize SPI port values
	DACset = DACdata[nloop];
	readword = spidac_tx(DACset); // write to DAC register
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void UserI2C() { // read I2 DACs for voltage and temperature for now
	...
	spill = int(J1[5]); // spill input
	sprintf(RXI2C, "%-6.2f %-6.3f %-6.1f\r\n",temp,volt,spill);
	writestring(fdnet, RXI2C);
...
void NMRMain() {
	...
	int odd; // sweep number even or odd
	//int nsweep = 19; // number of sweeps - 1
	int nloop2 = 0; // sweep loop counter
	int nloop3 = 0; // card loop counter
	int ADC1[1000]; // ADC values averaged over multiple sweeps
	double ADCsdev; // stand dev of voltage
	double SUMsdev; // sum of stand dev
	double ADCVoltage; // ADC Voltage
	static int first = 1; // initialization
	static unsigned char addr_state_adc = 0; //
	static unsigned char addr_state_dac = 0; //
	int sweep0; // 1 for first sweep
	int sweepend; // 1 for last sweep
	...
	// new DAC initialization here, performed once for each of 8 DAC cards
		  while (nloop3 < 8) {
			  	modadd = nloop3;
				addr_state = 0; // set bus direction control line (0x40) = 0 for Netburner write, FPGA read
				outporta(addr_state);   // initialize 8 bit address bus low, data bus xcvr as netburner output (low)
				addr_state = addr_state | (4*(modadd & a0)) | (modadd & a1) | ((modadd & a2)/4); // fixed for address swap
				addr_state = addr_state | aen; // set address enable
				outporta(addr_state);
				if (modadd == 2) addr_state_dac = addr_state; // save dac address
				UserDACinit();			// initialize
				nloop3++;
		  }
	...
	// Check front panel leds here, data bus high, address bus as set above, will be stable until command input
		...
		sscanf(RXBuffer,"%f %f %f %f %d %d %d %d %d %d %d %d %d", &volt3, &volt2, &volt4, &volt1, &adcnum, &nreq, &nfreq, &atten, &onoff, &gain, &modadd, &sweep0, &sweepend);
		// For FM mode, use DAC4 (logoff) to sweep the RF gen, voltage range -1 to +1V, Fcen = 0V
		// now use nreq as number of sweeps before averaging
		...
		// 1st check if input string is valid up to number of frequency steps, then talk to hardware
		  if (sweep0 == 1 ) { // detune and turn off RF from previous sweeps here
			  UserATTSW(0,0,60); // set RF switch off, low gain, max atten
			  UserDAC(dac2,6.,dac2,6.); // set tune to 6V, highest frequency
		  }
		if (nfreq >=1) { // got at least 7 arguments, so input probably OK
		  ...
		  // can comment out routines below to test individual hardware devices
			...
			addr_state = addr_state | (4*(modadd & a0)) | (modadd & a1) | ((modadd & a2)/4); // fixed for address swap
			addr_state = addr_state | aen; // set address enable
			addr_state_adc = addr_state; // save state
			outporta(addr_state);
		  ...
		  if (sweep0 == 1) OSTimeDly(TICKS_PER_SECOND / 2); // 0.25 sec settling time if tune changed
		  // OSTimeDly(TICKS_PER_SECOND / 4); // 0.25 sec settling time if tune changed
		  // nfreq = 100; // 100 freq values
		  // zero out sdev sume here
		  nloop = 0; // reset loop counter
//		  addr_state =  addr_state_dac;
//		  outporta(addr_state);
//		  UserDACFM(dac4,nfreq,nloop); // precompute DAC4 values
//		  addr_state =  addr_state_adc;
//		  outporta(addr_state);
		  //J1[6] = 1; // signal start
 		  /*while (nloop < nfreq) { // acquire an NMR spectrum
 			...
 			if (nloop == 0) {
 			   ADCsdev = UserADC(adcnum,10,nloop); // fix first data point
 			}
		    ...
		    nloop++;
		    UserDACFM(dac4,nfreq,nloop); // faster step to next freq by incrementing precomputed DAC4
 		  } */
 ...
 //		  while (nloop2 < nsweep) {
 		  while (nloop2 < (nreq)) {
 			 odd = nloop2 % 2;
 			 if (odd == 0) {  // even sweep #, sweep from low to high frequency
 				 nloop = 0; // reset loop counter
 				 addr_state =  addr_state_dac;
 				 outporta(addr_state);
 				 UserDACFM(dac4,nfreq,nloop); // select first precomputed DAC4 values
 				 addr_state =  addr_state_adc;
 				 outporta(addr_state);
 				 while (nloop < nfreq) { // acquire an NMR spectrum
 					 if (nloop == 0) {
 					 ...
 					 ADCsdev = UserADC(adcnum,1,nloop); // read ADC values for NMR output, returning computed mean and stand dev from nreq samples
 					 if (nloop2 == 0) {
 						 ADC1[nloop] = 0.;
 					 }
 					 ...
 					 addr_state =  addr_state_dac;
 					 outporta(addr_state);
 					 if (nloop < nfreq) UserDACFM(dac4,nfreq,nloop); // step to next freq by incrementing precomputed DAC4
 					 addr_state =  addr_state_adc;
 					 outporta(addr_state);
 				 }
 			 }
 			 else {  // odd sweep #, sweep from high to low frequency
 				 nloop = nfreq - 1; // reset loop counter
 				 addr_state =  addr_state_dac;
 				 outporta(addr_state);
 				 UserDACFM(dac4,nfreq,nloop); // select first precomputed DAC4 values
 				 addr_state =  addr_state_adc;
 				 outporta(addr_state);
 				 while (nloop >= 0) { // acquire an NMR spectrum
 					 if (nloop == (nfreq - 1)) {
 						 ADCsdev = UserADC(adcnum,10,nloop); // fix first data point
 					 }
 					 ADCsdev = UserADC(adcnum,1,nloop); // read ADC values for NMR output, returning computed mean and stand dev from nreq samples
 					 ...
 					 addr_state =  addr_state_dac;
 					 outporta(addr_state);
 					 if (nloop >= 0) UserDACFM(dac4,nfreq,nloop); // step to next freq by incrementing precomputed DAC4
 					 addr_state =  addr_state_adc;
 					 outporta(addr_state);
 				 }
 			 }
 		  ...
 		  nloop = 0;
 		 // UserDACFM(dac4,nfreq,0); // go back to first value (-1.)

 		  /*if (sweepend == 1) { // detune and turn off RF here
 			  UserATTSW(0,gain,atten); // set RF switch off
 			  UserDAC(dac2,6.,dac2,6.); // set tune to 6V
 		  } */
  		 ...
  		 *TXBuffer = '\0'; // start with empty string
		 int loop = 0; // char count
 		 while (nloop < nfreq) { // output loop
 //			ADC1[nloop] = ADC1[nloop] / (nsweep+1);
// 			ADC1[nloop] = ADC1[nloop] / (nreq);
// 			ADCVoltage = ((float)(ADC1[nloop]) / 65536.) * 20.; // calculate voltage (bipolar, +- 10V range)
 			ADCVoltage = ((float)(ADC1[nloop]) / 65536.) * 10.; // calculate voltage (bipolar, +- 5V range)
 			ADCVoltage = ADCVoltage / (float(nreq));
			sprintf(RXBuffer,"%-7.4f\r\n", ADCVoltage);
 			strcat(TXBuffer,RXBuffer); // try just concatenating strings and determine length afterwards
		    nloop++;
 		 }

 		loop = strlen(TXBuffer);
